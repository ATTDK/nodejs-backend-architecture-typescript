  ...
  try{
    ...
    for(let i=1;i<lists.length;i++){
      ...
      if($(lists[i]).attr('href')?.toString().match('dcinside') && !$(lists[i]).attr('href')?.toString().match('webcache')&& !$(lists[i]).attr('href')?.toString().match('recommend')){
        ...
        crawl_dc($(lists[i]).attr('href')!.toString(),artist)
      }
    }
    await page.close()
    if(next != undefined){
...
async function getDCUrl_google(index : number){
  switch (index) {
    case 1 : tt_gg_crawl1()
      break;
    case 2 : tt_gg_crawl2()
...
async function crawl_dc(url:string, artist : string) {
  var options ={
    url : url,
    ...
    request.get(options, async (req,res)=>{
      ...
      try{
        ...
        if (Content){
          ...
          result={
            title : $('h3 > span.title_subject').text().trim(),
            views : $('div.fr > span.gall_count').text().split(' ')[1],
            commentCount : $('div.fr > span.gall_comment > a').text().trim(),
            likes : $('div.fr > span.gall_reply_num').text().split(' ')[1],
            link : url,
            writer : $('div.fl > span.nickname').attr('title')!.toString(),
            created : $('div.fl > span.gall_date').attr('title')!.toString(),
            artist :artist,
            content : $('div.write_div').children().text().trim()
            } 
